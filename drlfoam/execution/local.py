...
def submit_and_wait(cmd: str, cwd: str):
...
class LocalBuffer(Buffer):
    def __init__(self, path: str, base_env: Environment, buffer_size: int,
                 n_runners_max: int, keep_trajectories: bool = True):
    ...
    def prepare(self):
        ...
        self._manager.add(submit_and_wait, cmd, cwd)
        self._manager.run()
    ...
    def fill(self):
        for env in self.envs:
            self._manager.add(
                submit_and_wait, f"./{env.run_script}", env.path
            )
        self._manager.run()
